...
function buildTrendEmailHtml(employeeName, weekKey, options = {}) {
    ...
    let emailText = `Performance Summary – ${weekStart}

Hi ${nickname},

Here's your performance summary for ${weekStart}.
`;
    ...
    metricsToAnalyze.forEach(metric => {
        ...
        emailText += `\n${statusIcon} ${metric.label}\n`;
        emailText += `   Your Value: ${employeeValue.toFixed(1)}${metric.unit}\n`;
        emailText += `   Center Avg: ${centerValue !== null ? centerValue.toFixed(1) + metric.unit : 'N/A'}\n`;
        emailText += `   Target: ${targetValue !== null ? targetValue + metric.unit : 'N/A'}\n`;
        emailText += `   vs. Center: ${vsCenter}\n`;
    });
    ...
    console.log('📧 Plain text email generated for', employeeName);
    
    return { htmlEmail: emailText, weekStart, subject: `Performance Summary – ${employeeName} – ${weekStart}` };
}
...
function generateExecutiveSummaryEmail() {
    ...
    let emailText = `YEAR-TO-DATE PERFORMANCE REVIEW
${associate}
Report Date: ${new Date().toLocaleDateString()}
${'='.repeat(60)}

SUMMARY RESULTS
✅ Meeting Target Goals: ${meetsTargetCount}/${totalMetricsEvaluated} (${Math.round((meetsTargetCount/totalMetricsEvaluated)*100)}% Success Rate)
📈 Outpacing Your Peers: ${outpacingPeersCount}/${totalMetricsEvaluated} (${Math.round((outpacingPeersCount/totalMetricsEvaluated)*100)}% Above Center)

${!hasSurveys ? 'NOTE: No survey data available for this period. Survey-based metrics are marked as meeting goals by default.\\n' : ''}

DETAILED PERFORMANCE METRICS
${'='.repeat(60)}
`;

    // Convert HTML table rows to plain text
    metricsHtml.split('<tr').forEach((row, idx) => {
    ...
    emailText += `Continue focusing on areas marked with ❌ and maintain excellence in areas marked with ✅\\n`;
    emailText += `\\nDisclaimer: This summary is close enough to use as a guide. YTD stats will be used in future decisions.`;
    
    // Display and copy
    const previewContainer = document.getElementById('executiveSummaryEmailPreview');
    if (previewContainer) {
        window.latestExecutiveSummaryHtml = emailText;
