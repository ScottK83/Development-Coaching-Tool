...
function renderEmployeeHistory() {
    ...
    let html = '<div style="margin-bottom: 20px;">';
    html += '<h3>Coaching History by Employee</h3>';
    html += '<p>View past coaching sessions for each employee.</p>';
    html += '</div>';
    
    const employees = Object.keys(coachingHistory).sort();
    
    if (employees.length === 0) {
        html += '<p style="color: #666; font-style: italic;">No coaching history yet. Generate some coaching emails to see them here!</p>';
    } else {
        employees.forEach(employeeName => {
            const sessions = coachingHistory[employeeName];
            
            html += `<div style="margin-bottom: 25px; padding: 15px; background: #f8f9fa; border-radius: 8px;">`;
            html += `<h3 style="color: #2196F3; margin-top: 0;">${escapeHtml(employeeName)}</h3>`;
            html += `<p style="color: #666;">Total Sessions: ${sessions.length}</p>`;
            
            sessions.slice().reverse().forEach((session, index) => {
                const date = new Date(session.date);
                html += `
                    <div style="margin-top: 15px; padding: 12px; background: white; border-radius: 4px; border-left: 4px solid #2196F3;">
                        ...
                        <details style="margin-top: 10px;">
                            <summary style="cursor: pointer; color: #2196F3;">View Email</summary>
                            <div style="margin-top: 10px; padding: 10px; background: #f8f9fa; border-radius: 4px; white-space: pre-wrap;">
