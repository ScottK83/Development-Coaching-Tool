...
// GLOBAL STATE
// ============================================
const APP_VERSION = '2026.09.23.30'; // Version: YYYY.MM.DD.NN
...
/**
 * Keep only the newest week-in-progress per Monday start date. A part-week is
 * re-uploaded over itself as the week fills in, so the older copies of one
 * Monday are genuinely superseded.
 *
 * Real YTD uploads are NOT pruned, and used to be: this kept one per year, on
 * the reasoning that an older snapshot was a stale duplicate cluttering the
 * period dropdown. A year-to-date file is a reading of a cumulative series, so
 * August's and September's are two points on that series and not two copies of
 * one thing. Deleting the older one made a YTD-over-YTD comparison impossible
 * to ever build: every upload silently destroyed the only thing it could have
 * been compared against, at startup, before anybody saw it.
 *
 * Nothing depended on there being one. Every surface that wants "the" YTD
 * already scans for the newest itself (center-ranking's _latestYtdKeyForYear,
 * morning-pulse's latestYtdPeriod, q1-review's getLatestYtdPeriod,
 * period-compare's _latestYtdReliability), which is why this could be removed
 * without a caller noticing. The dropdown concern is real and belongs to the
 * dropdown: the period chip rows already collapse past ten with "Show N more".
 *
 * ytdData is IndexedDB-backed, so retaining them is not against the 4MB
 * localStorage cap.
 */
function cleanupStaleDuplicatePeriods() {
    try {
        ...
        if (cleaned > 0) {
            ...
            console.log('[Startup] Cleaned ' + cleaned + ' superseded week-in-progress entries');
        }
    } catch (e) {
        console.warn('[Startup] Week-in-progress cleanup failed:', e.message);
    }
}
...
// A week-in-progress or month-to-date row is re-uploaded over itself as the
// period runs on, so an older copy with the same start date is replaced. Every
// other period type, real YTD included, is kept. Returns what was removed so
// undo can put it back.
function purgeSupersededPeriods(periodType, weekKey, startDate) {
    const purged = [];
    if (periodType !== 'week-in-progress' && periodType !== 'month-to-date') return purged;
    Object.keys(weeklyData).forEach(k => {
        ...
        if (startText !== startDate) return;
        const entry = { store: 'weekly', key: k, value: weeklyData[k], teamMembers: null };
        delete weeklyData[k];
        if (myTeamMembers && myTeamMembers[k]) {
            entry.teamMembers = myTeamMembers[k];
...
function appendPurgedToUploadUndo(purged) {
    const snap = loadUploadUndoSnapshot();
    if (!snap) return;
    snap.purged = (snap.purged || []).concat(purged);
    try {
        localStorage.setItem(UPLOAD_UNDO_STORAGE_KEY, JSON.stringify(snap));
    } catch (e) {
        console.warn('[undo] Failed to record purged periods:', e);
    }
}
...
function undoLastUpload() {
    ...
    let teamMembersRestored = false;
    (snap.purged || []).forEach(p => {
        const store = p.store === 'ytd' ? ytdData : weeklyData;
        store[p.key] = p.value;
        if (p.teamMembers && myTeamMembers) {
            myTeamMembers[p.key] = p.teamMembers;
            teamMembersRestored = true;
        }
    });
    if (teamMembersRestored) saveTeamMembers();
...
function handleLoadPastedDataClick() {
    ...
    try {
        ...
        // Only a re-uploaded part period is superseded. Real YTD uploads are a
        // cumulative series and every one is kept (see ytd-uploads-are-kept).
        const purged = purgeSupersededPeriods(periodType, weekKey, startDate);
        if (purged.length) {
            appendPurgedToUploadUndo(purged);
            if (purged.some(p => p.teamMembers)) saveTeamMembers();
        }
