...
// GLOBAL STATE
// ============================================
const APP_VERSION = '2026.03.26.8'; // Version: YYYY.MM.DD.NN
...
/**
 * Startup cleanup: remove stale auto-generated YTDs when a real YTD anchor exists.
 * Only removes auto-YTDs that have the SAME or OLDER end date as the anchor.
 * Keeps auto-YTDs that are NEWER than the anchor (they contain more recent data).
 */
function cleanupStaleAutoYtds() {
    try {
        ...
        years.forEach(year => {
            ...
            if (!anchor) return;
            const anchorEndTime = anchor.endDate ? anchor.endDate.getTime() : 0;
            ...
            Object.keys(ytdData).forEach(key => {
                ...
                const d = endText ? new Date(endText) : null;
                if (!d || isNaN(d) || d.getFullYear() !== year) return;
                // Only remove auto-YTDs that are same age or older than the anchor
                if (d.getTime() <= anchorEndTime) {
                    delete ytdData[key];
                    cleaned++;
                }
            });
...
function getLatestYearPeriodForEmployee(employeeName, reviewYear) {
    ...
    candidates.sort((a, b) => {
        ...
        // Most recent end date wins first — a newer auto-YTD should
        // beat a stale real YTD upload
        if (a.endDate.getTime() !== b.endDate.getTime()) {
            return b.endDate.getTime() - a.endDate.getTime();
        }
        // Same end date: prefer higher tier (real YTD > auto YTD > weekly)
