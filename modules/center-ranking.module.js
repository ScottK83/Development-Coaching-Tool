(function () {
    ...
    function buildCenterRankings() {
        ...
        // For center ranking the priority is having the MOST employees so we
        // can rank the entire call center. Check ALL sources (YTD + weekly)
        // and pick the period with the most employees. Newest date as tiebreaker.
        var bestPeriod = null;
        var bestCount = 0;
        var bestSource = '';
        var bestKey = '';
        ...
        function considerPeriod(key, period, sourceLabel) {
            ...
            // Pick the period with the most employees; tiebreak by newest date
            if (count > bestCount || (count === bestCount && endTime > bestDate)) {
                bestPeriod = period;
                bestCount = count;
                bestDate = endTime;
                bestSource = meta.label || sourceLabel || key;
                bestKey = key;
            }
        }
        ...
        Object.entries(yData).forEach(function (entry) {
            considerPeriod(entry[0], entry[1], 'YTD upload');
        });

        // Check weekly data too — full center data may live here
        Object.entries(wData).forEach(function (entry) {
            considerPeriod(entry[0], entry[1], entry[0]);
        });
