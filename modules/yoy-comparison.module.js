...
(function () {
    ...
    // Last year, whichever year that is. This was fixed at 2025 while the
    // current year came from the clock, so from 2027 it silently compared 2025
    // with 2027, and each January against a year with nothing in it.
    function _baselineYear() { return new Date().getFullYear() - 1; }
    function _baselineStart() { return _baselineYear() + '-01-01'; }
    function _baselineEnd() { return _baselineYear() + '-12-31'; }
    ...
    // A pasted baseline for last year wins. Without one, last year's newest
    // real YTD upload is used, which from 2027 on is already in the app. The
    // store keeps its old name; the year it holds is in its metadata, and a
    // baseline saved before that was recorded is the 2025 one.
    function _loadPastedBaseline() {
        try {
            var parsed = _storage()?.readStore?.('yoyBaseline2025');
            if (!parsed || !Array.isArray(parsed.employees)) return null;
            var year = parseInt(parsed.metadata && parsed.metadata.year, 10) || 2025;
            return year === _baselineYear() ? parsed : null;
        } catch (_e) { return null; }
    }

    function _baselineFromYtd() {
        var yData = _getYtdData();
        var year = _baselineYear();
        var best = null, bestEnd = '';
        Object.keys(yData).forEach(function (key) {
            var d = yData[key];
            var meta = (d && d.metadata) || {};
            if (meta.autoGeneratedYtd) return;
            var end = meta.endDate || (key.indexOf('|') >= 0 ? key.split('|')[1] : '');
            if (parseInt(String(end).slice(0, 4), 10) !== year) return;
            if (!d.employees || d.employees.length < 2) return;
            if (!best || String(end) > bestEnd) { best = d; bestEnd = String(end); }
        });
        if (!best) return null;
        return {
            employees: best.employees,
            metadata: { year: year, endDate: bestEnd, fromYtd: true, count: best.employees.length }
        };
    }

    function _loadBaseline() {
        return _loadPastedBaseline() || _baselineFromYtd();
    }
    ...
    function renderYoYComparison() {
        ...
        html += '<strong>📊 Year-over-Year</strong>, compare ' + _baselineYear() + ' vs ' + new Date().getFullYear() + ' per rep.';
        html += '<br><span style="color: var(--text-secondary); font-size: 0.85em;">Plug in your ' + _baselineYear() + ' full-year numbers below, then pick a metric to see who climbed and who slipped.</span>';
        html += '</div>';
        ...
        if (baseline) {
            ...
            var fromYtd = !!(baseline.metadata && baseline.metadata.fromYtd);
            html += '<span style="font-weight:600; color:var(--green-text);">✓ ' + _baselineYear() + ' baseline loaded'
                + (fromYtd ? ' from your YTD upload through ' + _escapeHtml(baseline.metadata.endDate || '') : '') + '</span>';
            html += '<span style="color:var(--text-secondary); font-size:0.9em;">' + baseline.employees.length + ' employees</span>';
            html += '<button type="button" id="yoyToggleInput" class="btn-secondary" style="font-size:0.85em; padding:5px 12px;">' + (fromYtd ? 'Paste different data' : 'Replace data') + '</button>';
            // A baseline read from the YTD upload has nothing pasted to clear.
            if (!fromYtd) html += '<button type="button" id="yoyClearBaseline" class="btn-secondary" style="background:#c62828; color:#fff; font-size:0.85em; padding:5px 12px;">Clear</button>';
            html += '</div>';
            html += '<div id="yoyInputArea" style="display:none; margin-top:12px;">';
        } else {
            html += '<div style="font-weight:600; margin-bottom:8px;">Plug in ' + _baselineYear() + ' stats</div>';
            html += '<div id="yoyInputArea">';
        }
        html += '<p style="margin:0 0 8px 0; color:var(--text-secondary); font-size:0.85em;">Paste your ' + _baselineYear() + ' full-year data (include the header row with <strong>Name</strong>). Same format as a YTD upload.</p>';
        ...
        html += '<button type="button" id="yoySaveBaseline" class="btn-primary" style="padding:6px 16px;">Save ' + _baselineYear() + ' Baseline</button>';
    ...
    function _bindInputPanel() {
        ...
        if (clearBtn) {
            clearBtn.addEventListener('click', function () {
                if (!window.confirm('Clear the ' + _baselineYear() + ' baseline? You will need to paste it again to compare.')) return;
                _clearBaseline();
                renderYoYComparison();
            });
        }
    ...
    function _onSaveBaseline() {
        ...
        var text = box.value || '';
        if (!text.trim()) {
            if (status) { status.style.color = '#c62828'; status.textContent = 'Paste your ' + _baselineYear() + ' data first.'; }
            return;
        }
        ...
        var employees;
        try {
            employees = parser.parsePastedData(text, _baselineStart(), _baselineEnd());
        } catch (err) {
    ...
    function _renderControlsAndTable() {
        ...
        html += '</label>';
        html += '<label style="font-size:0.9em; color:var(--text-secondary); cursor:pointer; user-select:none;">';
        html += '<input type="checkbox" id="yoyShow2025"' + (_show2025 ? ' checked' : '') + ' style="vertical-align:middle; margin-right:5px;">Show ' + _baselineYear() + ' numbers';
        html += '</label>';
    ...
    function _renderTable() {
        ...
        if (!data) {
            wrap.innerHTML = '<p style="color:var(--text-tertiary); text-align:center; padding:30px;">Upload current-year data to compare against your ' + _baselineYear() + ' baseline.</p>';
            return;
        }
        if (!data.rows.length) {
            wrap.innerHTML = '<p style="color:var(--text-tertiary); text-align:center; padding:30px;">No matching employees between ' + _baselineYear() + ' and ' + new Date().getFullYear() + '. Names must match across both data sets.</p>';
            return;
        }
        ...
        if (_show2025) html += '<th class="yoy-sort" data-sort="prior" style="' + thBase + ' text-align:right;">' + _baselineYear() + '' + arrow('prior') + '</th>';
    ...
    function _renderAllKpiTable() {
        ...
        html += '<strong>' + _baselineYear() + ' → ' + curYear + ' side-by-side</strong>, ' + data.matched + ' reps · click a KPI header to sort by who moved most. Click a value to see ' + _baselineYear() + ' vs ' + curYear + '.';
        html += '</div>';
    ...
    function _buildSummaryText() {
        ...
        if (!data || !data.rows.length) {
            return 'No year-over-year comparison data available. Load a ' + _baselineYear() + ' baseline and current-year data first.';
        }

        var lines = [];
        lines.push('Year-over-Year Performance Summary, ' + _baselineYear() + ' vs ' + curYear);
