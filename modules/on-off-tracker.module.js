(function () {
    ...
    function buildOnOffRowsHtml(rows) {
        return rows.map(row => `
                        <tr>
                            ...
                            <td style="padding: 8px; border: 1px solid #e3d7f7; font-size: 0.85em; color: var(--text-secondary);">${row.gapText || (row.score === 3 ? '✓' : '')}</td>
                        </tr>
                    `).join('');
    }
    ...
    function buildOnOffLegendMissingConfigCardHtml(label) {
        return `<div style="padding: 10px; border: 1px solid #e3d7f7; border-radius: 6px; background: var(--bg-surface);">
                <div style="font-weight: bold; color: #4a148c; margin-bottom: 4px;">${label}</div>
                <div style="font-size: 0.85em; color: var(--text-secondary);">No rating bands configured for selected year.</div>
            </div>`;
    }
    ...
    function buildOnOffLegendMinTypeCardHtml(metricKey, label, config) {
        ...
        return `<div style="padding: 10px; border: 1px solid #e3d7f7; border-radius: 6px; background: var(--bg-surface);">
                <div style="font-weight: bold; color: #4a148c; margin-bottom: 4px;">${label}</div>
                <div style="font-size: 0.85em; color: var(--green-text);">Score 3: \u2265 ${score3}</div>
                <div style="font-size: 0.85em; color: #8d6e00;">Score 2: &lt; ${score3} and \u2265 ${score2}</div>
                <div style="font-size: 0.85em; color: var(--red-text);">Score 1: &lt; ${score2}</div>
            </div>`;
    }
    ...
    function generateTeamOnOffSummary() {
        ...
        if (!employees.length) {
            container.innerHTML = '<p style="color: var(--text-tertiary); text-align: center;">No employees found. Upload data first.</p>';
            return;
        }
        ...
        if (!results.length) {
            container.innerHTML = '<p style="color: var(--text-tertiary); text-align: center;">No ' + reviewYear + ' data found for any team member.</p>';
            return;
        }
        ...
        // Build summary HTML
        var html = '<div style="background: var(--bg-surface); border-radius: 12px; padding: 20px; border: 2px solid #6a1b9a; ' +
        ...
        html += '<div style="border-bottom: 3px solid #6a1b9a; padding-bottom: 10px; margin-bottom: 16px;">' +
            ...
            '<div style="font-size: 0.85em; color: var(--text-secondary); margin-top: 4px;">' + results.length + ' associates evaluated</div></div>';
        ...
        html += '<div style="flex: 1; min-width: 140px; padding: 12px; border-radius: 8px; background: #c8f7c5; color: var(--green-text); text-align: center; border: 2px solid #66bb6a;">' +
        ...
        if (offTrack.length > 0) {
            ...
            html += '<div style="font-weight: 700; color: var(--red-text); margin-bottom: 8px; font-size: 0.95em;">Off Track (' + offTrack.length + ')</div>';
        ...
        [5, 4, 3, 2, 1, 0].forEach(function(count) {
            ...
            html += '<div style="padding: 10px 14px; margin-bottom: 6px; background: ' + bgColor + '; border-left: 4px solid ' + borderColor + '; border-radius: 4px;">' +
                ...
                '<span style="color: var(--text-secondary); font-size: 0.85em;"> (' + group.length + ')</span>' +
                '<div style="font-size: 0.85em; color: var(--text-secondary); margin-top: 4px;">' + names + '</div></div>';
        });
        ...
        results.forEach(function(r, idx) {
            ...
            function scoreCell(score, val, unit, metricKey) {
                ...
                if (score === null) return '<td style="padding: 4px 6px; text-align: center; color: var(--text-tertiary); border-bottom: 1px solid var(--border);"><div style="font-size: 0.75em; color: var(--text-tertiary);">' + valDisplay + '</div><div>--</div></td>';
                ...
                return '<td style="padding: 4px 6px; text-align: center; font-weight: 700; background: ' + cellBg + '; color: ' + cellColor + '; border-bottom: 1px solid var(--border);">' +
            ...
            html += '<tr style="background: ' + bg + ';">' +
                '<td style="padding: 6px 8px; font-weight: 600; border-bottom: 1px solid var(--border); white-space: nowrap;">' + r.firstName + '</td>' +
                ...
                '<td style="padding: 6px; text-align: center; font-weight: 700; border-bottom: 1px solid var(--border); color: #4a148c;">' + avgDisplay + '</td>' +
                '<td style="padding: 6px 10px; text-align: center; font-weight: 700; border-bottom: 1px solid var(--border); ' +
        ...
        // Legend
        html += '<div style="display: flex; gap: 12px; margin-top: 10px; padding: 8px 12px; background: var(--bg-surface-raised); border-radius: 6px; border: 1px solid var(--border); flex-wrap: wrap; font-size: 0.75em;">' +
    ...
    function _getMidYearMetaMap() {
        try {
            ...
            return readViaStorage('midYearMeta', prefix + 'midYearMeta', {}) || {};
        } catch (e) {
            return {};
        }
    }
