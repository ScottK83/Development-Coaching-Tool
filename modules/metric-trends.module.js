...
function upsertAutoYtdForYear(year, uptoEndDateText) {
    ...
    const yearNum = parseInt(year, 10);
    // Remove old auto-generated YTDs for this year
    ...
    // Never overwrite a real (non-auto) YTD upload with the same key
    const existingEntry = ytdData[aggregate.key];
    if (existingEntry && existingEntry.metadata && !existingEntry.metadata.autoGeneratedYtd) {
        // Real YTD exists at this key — don't overwrite it.
        // Store auto-YTD with a disambiguated key instead.
        const autoKey = aggregate.key + '|auto';
        ytdData[autoKey] = aggregate.entry;
        return;
    }
