...
(function() {
    ...
    /* Hours missed this year per person, from the newest year-to-date file.

       Built once and reused for a moment. The team table asked for one person
       at a time, and each ask re-read and re-normalized every YTD period, so a
       render did it 127 times over. It also kept whichever period the object
       happened to list last rather than the newest, so an older YTD could win.
       A real upload is preferred over an auto-built one, as everywhere else. */
    var _ytdReliabilityCache = { at: 0, map: null };

    function ytdReliabilityByName() {
        if (_ytdReliabilityCache.map && Date.now() - _ytdReliabilityCache.at < 2000) return _ytdReliabilityCache.map;
        var mod = window.DevCoachModules?.storage;
        var ytdData = (mod?.loadYtdData && mod.loadYtdData()) || {};
        var best = null, bestEnd = '', bestIsReal = false;
        Object.keys(ytdData).forEach(function(periodKey) {
            ...
            var end = String(meta.endDate || (periodKey.indexOf('|') > -1 ? periodKey.split('|')[1] : ''));
            var isReal = !meta.autoGeneratedYtd;
            if (!best || (isReal && !bestIsReal) || (isReal === bestIsReal && end > bestEnd)) {
                best = period; bestEnd = end; bestIsReal = isReal;
            }
        });
        var map = {};
        (best ? best.employees : []).forEach(function(emp) {
            var nameKey = buildEmployeeLookupKey(normalizeEmployeeName(emp?.name || ''));
            var val = Number(emp?.reliability);
            if (nameKey && isFinite(val)) map[nameKey] = val;
        });
        _ytdReliabilityCache = { at: Date.now(), map: map };
        return map;
    }
    ...
    function getYtdReliabilityHoursForEmployee(employeeName) {
        ...
        var val = ytdReliabilityByName()[targetKey];
        return val === undefined ? null : round2(val);
    }
