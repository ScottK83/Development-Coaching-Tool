...
(function () {
    ...
    function buildRankTimeline(scope, year) {
        ...
        /* The newest REAL year-to-date upload that had closed by the end of a
           given month, ranked.

           A reconstruction from the months on file is not the year when the file
           only goes back to May: it silently means "since May" while calling
           itself year to date, and it disagrees with the figure on the card,
           which comes from the uploaded year-to-date file. That file is the
           source of truth — it is the number that gets quoted back to people —
           so it wins wherever it exists, and the rebuild fills in only the months
           it does not reach.

           Auto-generated year-to-date rows are skipped: they are themselves built
           from the weeklies, so preferring one over the rebuild would dress the
           same reconstruction up as a source. */
        function _realYtdRankedAsOf(monthKey) {
            var yData = _ytdData();
            var best = null, bestEnd = '';
            Object.keys(yData).forEach(function (k) {
                var meta = (yData[k] && yData[k].metadata) || {};
                ...
                if (end.slice(0, 7) > monthKey) return;
                if (!best || end.localeCompare(bestEnd) > 0) { best = yData[k]; bestEnd = end; }
            });
            if (!best || !(best.employees || []).length) return null;
            var rows = _rank(best.employees, yr) || [];
            if (!rows.length) return null;
            var byName = {};
            rows.forEach(function (r) { byName[r.name] = r; });
            return { byName: byName, total: rows.length, end: bestEnd };
        }
        ...
        usable.forEach(function (per, i) {
            ...
            var real = _realYtdRankedAsOf(per.key);
            if (real) {
                cumulative[i] = {
                    byName: real.byName, total: real.total,
                    source: 'ytd-upload', coversFrom: null, through: real.end
                };
                return;
            }
            ...
            cumulative[i] = {
                byName: byName, total: Object.keys(byName).length,
                // Only the months on file, which may be nothing like the year.
                source: 'rebuilt', coversFrom: usable[0].key, through: per.key
            };
        });
        ...
        ranked.forEach(function (rows, i) {
            ...
            rows.forEach(function (r) {
                ...
                byName[r.name].push({
                    ...
                    // 'ytd-upload' means the uploaded file, which is what the card
                    // shows. 'rebuilt' means the months on file only, and
                    // overallCoversFrom says where those actually start.
                    overallSource: cumulative[i].source,
                    overallCoversFrom: cumulative[i].coversFrom,
                    overallThrough: cumulative[i].through,
